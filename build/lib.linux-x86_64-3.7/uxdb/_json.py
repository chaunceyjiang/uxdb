...
on uxdb.extension, so I can't create the default JSON typecasters in
...
from uxdb._psycopg import ISQLQuote, QuotedString
from uxdb._psycopg import new_type, new_array_type, register_type
from uxdb.compat import PY2
...
class Json(object):
    """
    An `~uxdb.extensions.ISQLQuote` wrapper to adapt a Python object to
    :sql:`json` data type.
...
def register_json(conn_or_curs=None, globally=False, loads=None,
    ...
    are provided.  Raise `~uxdb.ProgrammingError` if the type is not found.
...
def _get_json_oids(conn_or_curs, name='json'):
    # lazy imports
    from uxdb.extensions import STATUS_IN_TRANSACTION
    from uxdb.extras import _solve_conn_curs
