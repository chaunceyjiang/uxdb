...
psyco_connect(PyObject *self, PyObject *args, PyObject *keywds)
{
     ...
     * by uxdb. So, if not requiring an async conn, avoid passing
     * the async parameter. */
...
static struct {
    ...
    { "uxdb.Error", &Error, NULL, Error_doc },
    { "uxdb.Warning", &Warning, NULL, Warning_doc },
    { "uxdb.InterfaceError", &InterfaceError, &Error, InterfaceError_doc },
    { "uxdb.DatabaseError", &DatabaseError, &Error, DatabaseError_doc },
    { "uxdb.InternalError", &InternalError, &DatabaseError, InternalError_doc },
    { "uxdb.OperationalError", &OperationalError, &DatabaseError,
        OperationalError_doc },
    { "uxdb.ProgrammingError", &ProgrammingError, &DatabaseError,
        ProgrammingError_doc },
    { "uxdb.IntegrityError", &IntegrityError, &DatabaseError,
        IntegrityError_doc },
    { "uxdb.DataError", &DataError, &DatabaseError, DataError_doc },
    { "uxdb.NotSupportedError", &NotSupportedError, &DatabaseError,
        NotSupportedError_doc },
    { "uxdb.extensions.QueryCanceledError", &QueryCanceledError,
      &OperationalError, QueryCanceledError_doc },
    { "uxdb.extensions.TransactionRollbackError",
...
basic_errors_init(PyObject *module)
{
    /* the names of the exceptions here reflect the organization of the
       uxdb module and not the fact the original error objects live in
       _psycopg */
    ...
    if (!(errmodule = PyImport_ImportModule("uxdb.errors"))) {
...
sqlstate_errors_init(PyObject *module)
{
    ...
    for (i = 0; sqlstate_table[i].sqlstate; i++) {
        ...
        /* Expose the exceptions to uxdb.errors */
        if (errmodule) {
...
add_module_constants(PyObject *module)
{
    ...
    if (0 > PyModule_AddStringConstant(module,
        "__doc__", "uxdb PostgreSQL driver"))
    { return -1; }
