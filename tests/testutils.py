# testutils.py - utility module for uxdb testing.
...
from uxdb.compat import PY2, PY3, text_type
...
class ConnectingTestCase(unittest.TestCase):
    ...
    def connect(self, **kwargs):
        ...
        if 'dsn' in kwargs:
            ...
            conninfo = dsn
        conn = uxdb.connect(conninfo, **kwargs)
    ...
    def repl_connect(self, **kwargs):
        ...
        The connection is on "uxdb_TEST_REPL_DSN" unless overridden by
        a *dsn* kwarg.
        ...
        try:
            ...
            if conn.async_ == 1:
                self.wait(conn)
        except uxdb.OperationalError as e:
...
def libpq_version():
    v = uxdb.__libpq_version__
    if v >= 90100:
        v = min(v, uxdb.extensions.libpq_version())
    return v
...
def skip_if_no_superuser(f):
    ...
    def skip_if_no_superuser_(self):
        try:
            return f(self)
        except uxdb.errors.InsufficientPrivilege:
...
def slow(f):
    ...
    @wraps(f)
    def slow_(self):
        if os.environ.get('uxdb_TEST_FAST', '0') != '0':
...
def restore_types(f):
    ...
    def restore_types_(self):
        types = uxdb.extensions.string_types.copy()
        adapters = uxdb.extensions.adapters.copy()
        try:
            return f(self)
        finally:
            uxdb.extensions.string_types.clear()
            uxdb.extensions.string_types.update(types)
            uxdb.extensions.adapters.clear()
            uxdb.extensions.adapters.update(adapters)
