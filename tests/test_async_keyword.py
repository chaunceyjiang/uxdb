...
from uxdb.extras import LogicalReplicationConnection, StopReplication
...
class AsyncTests(ConnectingTestCase):
    ...
    def test_connection_setup(self):
        ...
        self.assert_(self.conn.encoding in uxdb.extensions.encodings)

    def test_async_subclass(self):
        class MyConn(uxdb.extensions.connection):
            def __init__(self, dsn, async=0):
                uxdb.extensions.connection.__init__(self, dsn, async=async)
    ...
    def test_async_connection_error_message(self):
        try:
            cnn = uxdb.connect('dbname=thisdatabasedoesntexist', async=True)
            self.wait(cnn)
...
class CancelTests(ConnectingTestCase):
    ...
    def test_async_cancel(self):
        async_conn = uxdb.connect(dsn, async=True)
        self.assertRaises(uxdb.OperationalError, async_conn.cancel)
        ...
        async_conn.cancel()
        self.assertRaises(uxdb.extensions.QueryCanceledError,
                          extras.wait_select, async_conn)
...
class ConnectTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self._connect_orig = uxdb._connect
        uxdb._connect = connect_stub
...
class AsyncReplicationTest(ReplicationTestCase):
    ...
    def test_async_replication(self):
        ...
        self.assertRaises(uxdb.ProgrammingError, cur.consume_stream, consume)
