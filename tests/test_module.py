...
class ConnectTestCase(unittest.TestCase):
    ...
    def test_supported_keywords(self):
        ...
        uxdb.connect(user='postgres')
        self.assertEqual(self.args[0], 'user=postgres')
        uxdb.connect(password='secret')
    ...
    def test_async(self):
        uxdb.connect(database='foo', host='baz', async_=1)
        self.assertDsnEqual(self.args[0], 'dbname=foo host=baz')
        self.assertEqual(self.args[1], None)
        self.assert_(self.args[2])

        uxdb.connect("dbname=foo host=baz", async_=True)
        self.assertDsnEqual(self.args[0], 'dbname=foo host=baz')
        self.assertEqual(self.args[1], None)
        self.assert_(self.args[2])
    ...
    def test_params_merging(self):
        ...
        uxdb.connect('dbname=foo', user='postgres')
...
class ExceptionsTestCase(ConnectingTestCase):
    ...
    def test_diagnostics_attributes(self):
        ...
        diag = e.diag
        self.assert_(isinstance(diag, uxdb.extensions.Diagnostics))
        for attr in [
...
class TestExtensionModule(unittest.TestCase):
    ...
    def test_import_internal(self):
        ...
        pkgdir = os.path.dirname(uxdb.__file__)
...
class TestVersionDiscovery(unittest.TestCase):
    def test_libpq_version(self):
        self.assertTrue(type(uxdb.__libpq_version__) is int)
        try:
            self.assertTrue(type(uxdb.extensions.libpq_version()) is int)
        except uxdb.NotSupportedError:
            self.assertTrue(uxdb.__libpq_version__ < 90100)
