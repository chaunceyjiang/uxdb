...
import uxdb.extras
from uxdb.extensions import POLL_OK, POLL_READ, POLL_WRITE
...
class GreenTestCase(ConnectingTestCase):
    def setUp(self):
        self._cb = uxdb.extensions.get_wait_callback()
        uxdb.extensions.set_wait_callback(uxdb.extras.wait_select)
        ConnectingTestCase.setUp(self)

    def tearDown(self):
        ConnectingTestCase.tearDown(self)
        uxdb.extensions.set_wait_callback(self._cb)

    def set_stub_wait_callback(self, conn, cb=None):
        stub = ConnectionStub(conn)
        uxdb.extensions.set_wait_callback(
            lambda conn: (cb or uxdb.extras.wait_select)(stub))
        return stub
    ...
    def test_flush_on_write(self):
        ...
        for mb in 1, 5, 10, 20, 50:
            ...
            if stub.polls.count(uxdb.extensions.POLL_WRITE) > 1:
                return
    ...
    def test_error_in_callback(self):
        ...
        uxdb.extensions.set_wait_callback(lambda conn: 1 // 0)
...
class CallbackErrorTestCase(ConnectingTestCase):
    def setUp(self):
        self._cb = uxdb.extensions.get_wait_callback()
        uxdb.extensions.set_wait_callback(self.crappy_callback)
        ConnectingTestCase.setUp(self)
        self.to_error = None
