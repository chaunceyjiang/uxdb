...
import uxdb
from uxdb.compat import lru_cache
import uxdb.extras
from uxdb.extras import NamedTupleConnection, NamedTupleCursor
...
class ExtrasDictCursorTests(_DictCursorBase):
    ...
    def testDictConnCursorArgs(self):
        self.conn.close()
        self.conn = self.connect(connection_factory=uxdb.extras.DictConnection)
        cur = self.conn.cursor()
        self.assert_(isinstance(cur, uxdb.extras.DictCursor))
        self.assertEqual(cur.name, None)
        # overridable
        cur = self.conn.cursor('foo',
            cursor_factory=uxdb.extras.NamedTupleCursor)
        self.assertEqual(cur.name, 'foo')
        self.assert_(isinstance(cur, uxdb.extras.NamedTupleCursor))
    ...
    def testDictCursorWithPlainCursorIterRowNumber(self):
        curs = self.conn.cursor(cursor_factory=uxdb.extras.DictCursor)
    ...
    def testDictCursorWithNamedCursorNotGreedy(self):
        curs = self.conn.cursor('tmp', cursor_factory=uxdb.extras.DictCursor)
    ...
    def _testWithNamedCursor(self, getter):
        curs = self.conn.cursor('aname', cursor_factory=uxdb.extras.DictCursor)
        curs.execute("SELECT * FROM ExtrasDictCursorTests")
        row = getter(curs)
        self.failUnless(row['foo'] == 'bar')
        self.failUnless(row[0] == 'bar')
...
class ExtrasDictCursorRealTests(_DictCursorBase):
    def testRealMeansReal(self):
        curs = self.conn.cursor(cursor_factory=uxdb.extras.RealDictCursor)
        curs.execute("SELECT * FROM ExtrasDictCursorTests")
        row = curs.fetchone()
    ...
    def testDictCursorRealWithNamedCursorNotGreedy(self):
        curs = self.conn.cursor('tmp', cursor_factory=uxdb.extras.RealDictCursor)
        self._testNamedCursorNotGreedy(curs)
    ...
    def _testWithNamedCursorReal(self, getter):
        curs = self.conn.cursor('aname',
            cursor_factory=uxdb.extras.RealDictCursor)
        curs.execute("SELECT * FROM ExtrasDictCursorTests")
        row = getter(curs)
        self.failUnless(row['foo'] == 'bar')
...
class NamedTupleCursorTest(ConnectingTestCase):
    ...
    def test_cursor_args(self):
        cur = self.conn.cursor('foo', cursor_factory=uxdb.extras.DictCursor)
        self.assertEqual(cur.name, 'foo')
        self.assert_(isinstance(cur, uxdb.extras.DictCursor))
    ...
    def test_no_result_no_surprise(self):
        ...
        self.assertRaises(uxdb.ProgrammingError, curs.fetchone)

        curs.execute("update nttest set s = s")
        self.assertRaises(uxdb.ProgrammingError, curs.fetchall)
