...
import uxdb
import uxdb.extras
from uxdb import extensions as ext
...
class ConnectionTests(ConnectingTestCase):
    ...
    def test_cleanup_on_badconn_close(self):
        ...
        conn = self.conn
        cur = conn.cursor()
        self.assertRaises(uxdb.OperationalError,
    ...
    def test_tpc_unsupported(self):
        ...
        self.assertRaises(uxdb.NotSupportedError,
            cnn.xid, 42, "foo", "bar")
    ...
    def test_connect_no_string(self):
        ...
        conn = uxdb.connect(MyString(dsn))
        conn.close()

    def test_weakref(self):
        conn = uxdb.connect(dsn)
        w = ref(conn)
        conn.close()
        del conn
        gc.collect()
    ...
    def test_connect_cursor_factory(self):
        conn = self.connect(cursor_factory=uxdb.extras.DictCursor)
        cur = conn.cursor()
    ...
    def test_cursor_factory(self):
        ...
        self.conn.cursor_factory = uxdb.extras.DictCursor
        self.assertEqual(self.conn.cursor_factory, uxdb.extras.DictCursor)
        cur = self.conn.cursor()
        cur.execute("select 1 as a")
        self.assertEqual(cur.fetchone()['a'], 1)
    ...
    def test_cursor_factory_none(self):
        ...
        conn = self.connect(cursor_factory=uxdb.extras.DictCursor)
        cur = conn.cursor(cursor_factory=None)
        self.assertEqual(type(cur), uxdb.extras.DictCursor)
...
class ParseDsnTestCase(ConnectingTestCase):
    def test_parse_dsn(self):
        ...
        self.assertRaises(uxdb.ProgrammingError, ext.parse_dsn,
        ...
        try:
            ...
            ext.parse_dsn("dbname='test 2 user=tester password=secret")
        except uxdb.ProgrammingError as e:
            raised = True
...
class IsolationLevelsTestCase(ConnectingTestCase):
    ...
    def test_set_isolation_level_abort(self):
        ...
        conn.set_isolation_level(
            uxdb.extensions.ISOLATION_LEVEL_SERIALIZABLE)
        self.assertEqual(uxdb.extensions.TRANSACTION_STATUS_IDLE,
            conn.info.transaction_status)
        cur.execute("select count(*) from isolevel;")
        self.assertEqual(0, cur.fetchone()[0])

        cur.execute("insert into isolevel values (10);")
        self.assertEqual(uxdb.extensions.TRANSACTION_STATUS_INTRANS,
            conn.info.transaction_status)
        conn.set_isolation_level(
            uxdb.extensions.ISOLATION_LEVEL_AUTOCOMMIT)
        self.assertEqual(uxdb.extensions.TRANSACTION_STATUS_IDLE,
            conn.info.transaction_status)
        cur.execute("select count(*) from isolevel;")
        self.assertEqual(0, cur.fetchone()[0])
        ...
        self.assertEqual(uxdb.extensions.TRANSACTION_STATUS_IDLE,
            conn.info.transaction_status)
        conn.set_isolation_level(
            uxdb.extensions.ISOLATION_LEVEL_READ_COMMITTED)
        self.assertEqual(uxdb.extensions.TRANSACTION_STATUS_IDLE,
            conn.info.transaction_status)
        cur.execute("select count(*) from isolevel;")
...
class ConnectionTwoPhaseTests(ConnectingTestCase):
    ...
    def test_cancel_fails_prepared(self):
        ...
        cnn.tpc_prepare()
        self.assertRaises(uxdb.ProgrammingError, cnn.cancel)

    def test_tpc_recover_non_dbapi_connection(self):
        cnn = self.connect(connection_factory=uxdb.extras.RealDictConnection)
...
class TestEncryptPassword(ConnectingTestCase):
    ...
    def test_encrypt_password_post_9_6(self):
        # MD5 algorithm
        self.assertEqual(
            ext.encrypt_password('uxdb', 'ashesh', self.conn, 'md5'),
            'md594839d658c28a357126f105b9cb14cfc')

        # keywords
        self.assertEqual(
            ext.encrypt_password(
                password='uxdb', user='ashesh',
    ...
    def test_encrypt_server(self):
        ...
        self.assertEqual(
            ext.encrypt_password(
                'uxdb', 'ashesh', self.conn, 'scram-sha-256'
            )[:14], 'SCRAM-SHA-256$')

        self.assertRaises(uxdb.ProgrammingError,
            ext.encrypt_password, 'uxdb', 'ashesh', self.conn, 'abc')

    def test_encrypt_md5(self):
        self.assertEqual(
            ext.encrypt_password('uxdb', 'ashesh', algorithm='md5'),
            'md594839d658c28a357126f105b9cb14cfc')
...
class PasswordLeakTestCase(ConnectingTestCase):
    ...
    def test_leak(self):
        self.assertRaises(uxdb.DatabaseError,
...
class TestConnectionInfo(ConnectingTestCase):
    def setUp(self):
        ...
        class BrokenConn(uxdb.extensions.connection):
            def __init__(self, *args, **kwargs):
    ...
    def test_ssl_not_supported(self):
        with self.assertRaises(uxdb.NotSupportedError):
