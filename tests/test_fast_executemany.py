...
class TestPaginate(unittest.TestCase):
    def test_paginate(self):
        def pag(seq):
            return uxdb.extras._paginate(seq, 100)
...
class TestExecuteValues(FastExecuteTestMixin, testutils.ConnectingTestCase):
    def test_empty(self):
        cur = self.conn.cursor()
        uxdb.extras.execute_values(cur,
    ...
    def test_returning(self):
        cur = self.conn.cursor()
        result = uxdb.extras.execute_values(cur,
    ...
    def test_invalid_sql(self):
        cur = self.conn.cursor()
        self.assertRaises(ValueError, uxdb.extras.execute_values, cur,
            "insert", [])
        self.assertRaises(ValueError, uxdb.extras.execute_values, cur,
            "insert %s and %s", [])
        self.assertRaises(ValueError, uxdb.extras.execute_values, cur,
