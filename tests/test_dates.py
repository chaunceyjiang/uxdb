...
import uxdb
from uxdb.tz import FixedOffsetTimezone, ZERO
import unittest
...
class CommonDatetimeTestsMixin:
    ...
    def test_parse_incomplete_date(self):
        self.assertRaises(uxdb.DataError, self.DATE, '2007', self.curs)
        self.assertRaises(uxdb.DataError, self.DATE, '2007-01', self.curs)
    ...
    def test_parse_incomplete_time(self):
        self.assertRaises(uxdb.DataError, self.TIME, '13', self.curs)
        self.assertRaises(uxdb.DataError, self.TIME, '13:30', self.curs)
    ...
    def test_parse_incomplete_datetime(self):
        self.assertRaises(uxdb.DataError,
...
class DatetimeTests(ConnectingTestCase, CommonDatetimeTestsMixin):
    """Tests for the datetime based date handling in uxdb."""

    def setUp(self):
        ConnectingTestCase.setUp(self)
        self.curs = self.conn.cursor()
        self.DATE = uxdb.extensions.PYDATE
        self.TIME = uxdb.extensions.PYTIME
        self.DATETIME = uxdb.extensions.PYDATETIME
        self.INTERVAL = uxdb.extensions.PYINTERVAL
    ...
    def test_interval_overflow(self):
        ...
        uxdb.extensions.register_type(
            uxdb.extensions.new_type(
                uxdb.STRING.values, 'WAT', uxdb.extensions.INTERVAL),
            cur)
    ...
    def test_interval_iso_8601_not_supported(self):
        ...
        self.assertRaises(uxdb.NotSupportedError, cur.fetchone)


@unittest.skipUnless(
    hasattr(uxdb._psycopg, 'MXDATETIME'),
    'Requires mx.DateTime support'
)
class mxDateTimeTests(ConnectingTestCase, CommonDatetimeTestsMixin):
    """Tests for the mx.DateTime based date handling in uxdb."""

    def setUp(self):
        ConnectingTestCase.setUp(self)
        self.curs = self.conn.cursor()
        self.DATE = uxdb._psycopg.MXDATE
        self.TIME = uxdb._psycopg.MXTIME
        self.DATETIME = uxdb._psycopg.MXDATETIME
        self.INTERVAL = uxdb._psycopg.MXINTERVAL

        uxdb.extensions.register_type(self.DATE, self.conn)
        uxdb.extensions.register_type(self.TIME, self.conn)
        uxdb.extensions.register_type(self.DATETIME, self.conn)
        uxdb.extensions.register_type(self.INTERVAL, self.conn)
        uxdb.extensions.register_type(uxdb.extensions.MXDATEARRAY, self.conn)
        uxdb.extensions.register_type(uxdb.extensions.MXTIMEARRAY, self.conn)
        uxdb.extensions.register_type(
            uxdb.extensions.MXDATETIMEARRAY, self.conn)
        uxdb.extensions.register_type(
            uxdb.extensions.MXINTERVALARRAY, self.conn)
...
class FixedOffsetTimezoneTests(unittest.TestCase):
    ...
    def test_repr_with_positive_offset(self):
        ...
        self.assertEqual(repr(tzinfo),
            "uxdb.tz.FixedOffsetTimezone(offset=300, name=None)")
    ...
    def test_repr_with_name(self):
        ...
        self.assertEqual(repr(tzinfo),
            "uxdb.tz.FixedOffsetTimezone(offset=0, name='FOO')")
