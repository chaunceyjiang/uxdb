...
import uxdb
from uxdb.extensions import AsIs, adapt, register_adapter
...
class TypesBasicTests(ConnectingTestCase):
    ...
    def testBinaryRoundTrip(self):
        # test to make sure buffers returned by uxdb are
        # understood by execute:
        if PY2:
            s = ''.join([chr(x) for x in range(256)])
            buf = self.execute("SELECT %s::bytea AS foo", (uxdb.Binary(s),))
    ...
    def testArrayMalformed(self):
        ...
        for s in ss:
            self.assertRaises(uxdb.DataError,
                uxdb.extensions.STRINGARRAY, s.encode('utf8'), curs)
    ...
    def testUnicodeArray(self):
        uxdb.extensions.register_type(
            uxdb.extensions.UNICODEARRAY, self.conn)
        curs = self.conn.cursor()
        curs.execute("select '{a,b,c}'::text[]")
        x = curs.fetchone()[0]
    ...
    def testBytesArray(self):
        uxdb.extensions.register_type(
            uxdb.extensions.BYTESARRAY, self.conn)
        curs = self.conn.cursor()
        curs.execute("select '{a,b,c}'::text[]")
        x = curs.fetchone()[0]
    ...
    def testGenericArrayNull(self):
        def caster(s, cur):
            if s is None:
                return "nada"
            return int(s) * 2
        base = uxdb.extensions.new_type((23,), "INT4", caster)
        array = uxdb.extensions.new_array_type((1007,), "INT4ARRAY", base)

        uxdb.extensions.register_type(array, self.conn)
        a = self.execute("select '{1, 2, 3}'::int4[]")
...
class AdaptSubclassTest(unittest.TestCase):
    ...
    def test_no_mro_no_joy(self):
        ...
        register_adapter(A, lambda a: AsIs("a"))
        self.assertRaises(uxdb.ProgrammingError, adapt, B())
...
class ByteaParserTest(unittest.TestCase):
    ...
    def _import_cast(self):
        ...
        lib = ctypes.pydll.LoadLibrary(uxdb._psycopg.__file__)
