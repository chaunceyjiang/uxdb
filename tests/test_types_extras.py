...
import uxdb
import uxdb.extras
import uxdb.extensions as ext
from uxdb._json import _get_json_oids
from uxdb.extras import (
...
from uxdb.tz import FixedOffsetTimezone
...
class TypesExtrasTests(ConnectingTestCase):
    ...
    def testINET(self):
        with warnings.catch_warnings():
            ...
            uxdb.extras.register_inet()

        i = uxdb.extras.Inet("192.168.1.0/24")
    ...
    def test_adapt_fail(self):
        class Foo(object):
            pass
        self.assertRaises(uxdb.ProgrammingError,
            uxdb.extensions.adapt, Foo(), ext.ISQLQuote, None)
        try:
            uxdb.extensions.adapt(Foo(), ext.ISQLQuote, None)
        except uxdb.ProgrammingError as err:
            self.failUnless(str(err) == "can't adapt type 'Foo'")

    def test_point_array(self):
        # make sure a point array is never casted to a float array,
        # see https://github.com/psycopg/uxdb/issues/613
...
class HstoreTestCase(ConnectingTestCase):
    def test_adapt_8(self):
        ...
        if 'd' in o:
            encc = u'\xe0'.encode(uxdb.extensions.encodings[self.conn.encoding])
    ...
    def test_parse(self):
        ...
        def ko(s):
            self.assertRaises(uxdb.InterfaceError,
...
class AdaptTypeTestCase(ConnectingTestCase):
    ...
    def test_cast_composite(self):
        ...
        t = uxdb.extras.register_composite("type_isd", self.conn)
    ...
    def test_empty_string(self):
        ...
        curs = self.conn.cursor()
        uxdb.extras.register_composite("type_ss", curs)
    ...
    def test_cast_nested(self):
        ...
        uxdb.extras.register_composite("type_is", self.conn)
        uxdb.extras.register_composite("type_r_dt", self.conn)
        uxdb.extras.register_composite("type_r_ft", self.conn)
    ...
    def test_register_on_cursor(self):
        ...
        uxdb.extras.register_composite("type_ii", curs1)
    ...
    def test_register_on_connection(self):
        ...
        conn2 = self.connect()
        try:
            uxdb.extras.register_composite("type_ii", conn1)
            curs1 = conn1.cursor()
            curs2 = conn2.cursor()
            curs1.execute("select (1,2)::type_ii")
            self.assertEqual(curs1.fetchone()[0], (1, 2))
    ...
    def test_register_globally(self):
        ...
        conn2 = self.connect()
        try:
            uxdb.extras.register_composite("type_ii", conn1, globally=True)
            curs1 = conn1.cursor()
            curs2 = conn2.cursor()
            curs1.execute("select (1,2)::type_ii")
            self.assertEqual(curs1.fetchone()[0], (1, 2))
    ...
    def test_composite_namespace(self):
        ...
        self._create_type("typens.typens_ii",
            [("a", "integer"), ("b", "integer")])
        t = uxdb.extras.register_composite(
    ...
    def test_wrong_schema(self):
        ...
        curs = self.conn.cursor()
        uxdb.extensions.register_type(c.typecaster, curs)
        curs.execute("select (1,2)::type_ii")
        self.assertRaises(uxdb.DataError, curs.fetchone)
    ...
    def test_from_tables(self):
        ...
        uxdb.extras.register_composite("ctest1", curs)
        uxdb.extras.register_composite("ctest2", curs)
...
class JsonTestCase(ConnectingTestCase):
    def test_adapt(self):
        ...
        for obj in enumerate(objs):
            self.assertQuotedEqual(curs.mogrify("%s", (Json(obj),)),
                uxdb.extensions.QuotedString(json.dumps(obj)).getquoted())
    ...
    def test_register_on_dict(self):
        uxdb.extensions.register_adapter(dict, Json)
    ...
    def test_type_not_available(self):
        ...
        self.assertRaises(uxdb.ProgrammingError,
            uxdb.extras.register_json, self.conn)
    ...
    def test_register_on_cursor(self):
        curs = self.conn.cursor()
        uxdb.extras.register_json(curs)
        curs.execute("""select '{"a": 100.0, "b": null}'::json""")
        self.assertEqual(curs.fetchone()[0], {'a': 100.0, 'b': None})
    ...
    @restore_types
    def test_register_globally(self):
        new, newa = uxdb.extras.register_json(self.conn, globally=True)
        curs = self.conn.cursor()
        curs.execute("""select '{"a": 100.0, "b": null}'::json""")
        self.assertEqual(curs.fetchone()[0], {'a': 100.0, 'b': None})
    ...
    @skip_if_no_json_type
    def test_loads(self):
        json = uxdb.extras.json

        def loads(s):
            return json.loads(s, parse_float=Decimal)
        uxdb.extras.register_json(self.conn, loads=loads)
        curs = self.conn.cursor()
        curs.execute("""select '{"a": 100.0, "b": null}'::json""")
    ...
    def test_no_conn_curs(self):
        ...
        def loads(s):
            return uxdb.extras.json.loads(s, parse_float=Decimal)

        new, newa = uxdb.extras.register_json(
    ...
    def test_register_default(self):
        ...
        def loads(s):
            return uxdb.extras.json.loads(s, parse_float=Decimal)
        uxdb.extras.register_default_json(curs, loads=loads)
    ...
    def test_no_array_oid(self):
        curs = self.conn.cursor()
        t1, t2 = uxdb.extras.register_json(curs, oid=25)
    ...
    def test_str(self):
        snowman = u"\u2603"
        obj = {'a': [1, 2, snowman]}
        j = uxdb.extensions.adapt(uxdb.extras.Json(obj))
        s = str(j)
...
class JsonbTestCase(ConnectingTestCase):
    ...
    def test_register_on_connection(self):
        uxdb.extras.register_json(self.conn, loads=self.myloads, name='jsonb')
        curs = self.conn.cursor()
        curs.execute("""select '{"a": 100.0, "b": null}'::jsonb""")
    ...
    def test_register_on_cursor(self):
        curs = self.conn.cursor()
        uxdb.extras.register_json(curs, loads=self.myloads, name='jsonb')
        curs.execute("""select '{"a": 100.0, "b": null}'::jsonb""")
        self.assertEqual(curs.fetchone()[0], {'a': 100.0, 'b': None, 'test': 1})

    @restore_types
    def test_register_globally(self):
        new, newa = uxdb.extras.register_json(self.conn,
    ...
    def test_loads(self):
        ...
        uxdb.extras.register_json(self.conn, loads=loads, name='jsonb')
        curs = self.conn.cursor()
        curs.execute("""select '{"a": 100.0, "b": null}'::jsonb""")
        data = curs.fetchone()[0]
        self.assert_(isinstance(data['a'], Decimal))
    ...
    def test_register_default(self):
        ...
        uxdb.extras.register_default_jsonb(curs, loads=loads)
