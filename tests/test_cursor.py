...
import uxdb.extras
from uxdb.compat import text_type
...
class CursorTests(ConnectingTestCase):
    ...
    def test_empty_query(self):
        cur = self.conn.cursor()
        self.assertRaises(uxdb.ProgrammingError, cur.execute, "")
        self.assertRaises(uxdb.ProgrammingError, cur.execute, " ")
        self.assertRaises(uxdb.ProgrammingError, cur.execute, ";")
    ...
    def test_cast_specificity(self):
        ...
        D = uxdb.extensions.new_type((705,), "DOUBLING", lambda v, c: v * 2)
        uxdb.extensions.register_type(D, self.conn)
        self.assertEqual("foofoo", curs.cast(705, 'foo'))

        T = uxdb.extensions.new_type((705,), "TREBLING", lambda v, c: v * 3)
        uxdb.extensions.register_type(T, curs)
    ...
    def test_withhold(self):
        self.assertRaises(uxdb.ProgrammingError, self.conn.cursor,
                          withhold=True)
    ...
    def test_withhold_no_begin(self):
        ...
        self.assertEqual(self.conn.status, uxdb.extensions.STATUS_BEGIN)
        self.assertEqual(self.conn.info.transaction_status,
                         uxdb.extensions.TRANSACTION_STATUS_INTRANS)

        self.conn.commit()
        self.assertEqual(self.conn.status, uxdb.extensions.STATUS_READY)
        self.assertEqual(self.conn.info.transaction_status,
                         uxdb.extensions.TRANSACTION_STATUS_IDLE)
    ...
    def test_not_scrollable(self):
        ...
        curs.execute("select * from scrollable")
        curs.scroll(2)
        self.assertRaises(uxdb.OperationalError, curs.scroll, -1)
    ...
    def test_description_attribs(self):
        ...
        self.assert_(c.type_code in uxdb.extensions.DECIMAL.values)
        ...
        self.assert_(c.type_code in uxdb.STRING.values)
        self.assert_(c.internal_size < 0)
        ...
        self.assert_(c.type_code in uxdb.extensions.DATE.values)
        self.assert_(c.internal_size > 0)
        self.assertEqual(c.precision, None)
        self.assertEqual(c.scale, None)
    ...
    def test_scroll(self):
        ...
        self.assertRaises((IndexError, uxdb.ProgrammingError),
    ...
    def test_bad_subclass(self):
        ...
        class StupidCursor(uxdb.extensions.cursor):
            def __init__(self, *args, **kwargs):
                # I am stupid so not calling superclass init
                pass

        cur = StupidCursor()
        self.assertRaises(uxdb.InterfaceError, cur.execute, 'select 1')
        self.assertRaises(uxdb.InterfaceError, cur.executemany,
            'select 1', [])
    ...
    # DBAPI2.0 test section because they are a uxdb extension.
    ...
    def test_callproc_dict(self):
        ...
        failing_cases = [
            ({paramname: 2, 'foo': 'bar'}, uxdb.ProgrammingError),
            ({paramname: '2'}, uxdb.ProgrammingError),
            ({paramname: 'two'}, uxdb.ProgrammingError),
            ({u'bj\xc3rn': 2}, uxdb.ProgrammingError),
            ({3: 2}, TypeError),
            ({self: 2}, TypeError),
        ]
    ...
    def test_external_close_sync(self):
        ...
        # behind uxdb's back, uxdb always handles it correctly:
    ...
    def test_external_close_async(self):
        ...
        wait_func = uxdb.extras.wait_select
        self._test_external_close(control_conn, connect_func, wait_func)
    ...
    def _test_external_close(self, control_conn, connect_func, wait_func):
        ...
        for i in range(10):
            ...
            self.assertRaises(uxdb.OperationalError, f)
