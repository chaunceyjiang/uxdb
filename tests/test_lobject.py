...
class LargeObjectTests(LargeObjectTestCase):
    ...
    def test_open_non_existent(self):
        ...
        lo = self.conn.lobject()
        lo.unlink()
        self.assertRaises(uxdb.OperationalError, self.conn.lobject, lo.oid)
    ...
    def test_write_after_close(self):
        lo = self.conn.lobject()
        lo.close()
        self.assertRaises(uxdb.InterfaceError, lo.write, b"some data")

    def test_read_after_close(self):
        lo = self.conn.lobject()
        lo.close()
        self.assertRaises(uxdb.InterfaceError, lo.read, 5)

    def test_seek_after_close(self):
        lo = self.conn.lobject()
        lo.close()
        self.assertRaises(uxdb.InterfaceError, lo.seek, 0)

    def test_tell_after_close(self):
        lo = self.conn.lobject()
        lo.close()
        self.assertRaises(uxdb.InterfaceError, lo.tell)
    ...
    def test_write_after_commit(self):
        ...
        self.assertRaises(uxdb.ProgrammingError, lo.write, b"some data")
    ...
    def test_read_after_commit(self):
        ...
        self.assertRaises(uxdb.ProgrammingError, lo.read, 5)
    ...
    def test_seek_after_commit(self):
        ...
        self.assertRaises(uxdb.ProgrammingError, lo.seek, 0)
    ...
    def test_tell_after_commit(self):
        ...
        self.assertRaises(uxdb.ProgrammingError, lo.tell)
    ...
    def test_factory(self):
        class lobject_subclass(uxdb.extensions.lobject):
            pass
...
def skip_if_no_truncate(f):
    ...
    def skip_if_no_truncate_(self):
        ...
        if not hasattr(uxdb.extensions.lobject, 'truncate'):
            return self.skipTest(
                "uxdb has been built against a libpq "
...
class LargeObjectTruncateTests(LargeObjectTestCase):
    ...
    def test_truncate_after_close(self):
        lo = self.conn.lobject()
        lo.close()
        self.assertRaises(uxdb.InterfaceError, lo.truncate)
    ...
    def test_truncate_after_commit(self):
        ...
        self.assertRaises(uxdb.ProgrammingError, lo.truncate)
...
def _has_lo64(conn):
    ...
    if 'lo64' not in uxdb.__version__:
...
class LargeObjectNot64Tests(LargeObjectTestCase):
    def test_seek_larger_than_2gb(self):
        ...
        offset = 1 << 32  # 4gb
        self.assertRaises(
            (OverflowError, uxdb.InterfaceError, uxdb.NotSupportedError),
            lo.seek, offset, 0)
