...
from uxdb.extensions import adapt, quote_ident
...
class QuotingTestCase(ConnectingTestCase):
    ...
    def test_binary(self):
        ...
        curs = self.conn.cursor()
        curs.execute("SELECT %s::bytea;", (uxdb.Binary(data),))
        if PY2:
    ...
    def test_unicode(self):
        ...
        uxdb.extensions.register_type(uxdb.extensions.UNICODE, self.conn)
    ...
    def test_bytes(self):
        ...
        uxdb.extensions.register_type(uxdb.extensions.BYTES, conn)
        curs = conn.cursor()
...
class TestQuotedString(ConnectingTestCase):
    def test_encoding_from_conn(self):
        q = uxdb.extensions.QuotedString('hi')
...
class TestStringAdapter(ConnectingTestCase):
    ...
    def test_adapt_bytes(self):
        snowman = u"\u2603"
        self.conn.set_client_encoding('utf8')
        a = uxdb.extensions.QuotedString(snowman.encode('utf8'))
        a.prepare(self.conn)
        self.assertQuotedEqual(a.getquoted(), b"'\xe2\x98\x83'")
